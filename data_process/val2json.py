...
parser.add_argument('--root_dir', default='/data2/logo/qrcode/all',type=str, help="root path of images and labels, include ./images and ./labels and classes.txt")
...
def yolo2coco(arg):
    ...
    originLabelsDir = os.path.join(root_path, 'labels_val')                                        
    originImagesDir = os.path.join(root_path, 'images_val')
    with open(os.path.join(root_path, 'label_class.txt')) as f:
    ...
    for i, cls in enumerate(classes, 0):
        dataset['categories'].append({'id': i+1, 'name': cls, 'supercategory': 'logo'})
    ...
    for k, index in enumerate(tqdm(indexes)):
        ...
        # 读取图像的宽和高
        im = cv2.imread(os.path.join(root_path, 'images_val/') + index)
